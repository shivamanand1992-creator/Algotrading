...
Simulates Hermes trading decisions and calculates performance metrics
"""
...
class HermesBacktester:
    ...
    def __init__(self, capital_per_trade: float = 10000, max_trades_per_day: int = 4):
        ...
        # Enhanced features (matching live config)
    ...
    async def simulate_trade_decision(self, market_data: Dict) -> Dict:
        """Simulate Hermes decision (in backtest, simplified without LLM)"""
        # Simplified decision logic for backtesting (rule-based)
        ...
        if rsi < 40 and macd > 0:
            action = "BUY"
            confidence = 0.75
        elif rsi > 30 and rsi < 45 and macd > 0.1:
            action = "BUY"
            confidence = 0.72
        # Bearish conditions (exit signal)
        elif rsi > 60 or macd < -0.1:
    ...
    async def backtest_day(self, df_day: pd.DataFrame, date: str) -> Dict:
        ...
        for idx, row in df_trading.iterrows():
            ...
            decision = await self.simulate_trade_decision(market_data)
    ...
    async def run_backtest(self, start_date: str, end_date: str):
        ...
        for date in unique_dates:
            ...
            result = await self.backtest_day(df_day, str(date))

            if result['trades'] > 0:
                self.daily_stats.append(result)
                self.trades.extend(result['trades_data'])

                print(f"{date} | Trades: {result['trades']} | P&L: ₹{result['pnl']:>8.2f}")
